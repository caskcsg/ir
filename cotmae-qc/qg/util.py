...
def write_to_tsv(output_file: str, data: Dict[int, List[str]]):
    with open(output_file, 'w') as fOut:
        writer = csv.writer(fOut, delimiter="\t", quoting=csv.QUOTE_MINIMAL)
        for doc_id in sorted(data.keys()):
            query_set = data[doc_id]
            writer.writerow(query_set)

def write_to_json_for_cotmae(output_file: str, data: Dict[str, str], corpus: Dict[str, Dict[str, str]]):
    with open(output_file, 'w') as fOut:
        for idx, text in data.items():
            json.dump({
                "docid": corpus[idx]["docid"],
                "anchor": corpus[idx]["text"],
                "random_sampled": corpus[idx]["random_sampled"],
                "nearby": corpus[idx]["nearby"],
                "overlap": corpus[idx]["overlap"],
                "query": text,
